...
public function getPrice(Request $request)
{
    ...
    $apiKey = Setting::get('smspool_api_key');
    ...
    $response = Http::asForm()->post('https://api.smspool.net/request/pricing', $payload);
    $json = $response->json();
    
    Log::info('Server 3 getPrice() response', [
        'payload' => $payload,
        'response' => $json
    ]);
    ...
    if (!is_array($json) || empty($json)) {
        return response()->json([
            'success' => false,
            'message' => 'No pricing available for this service/country combination.',
        ]);
    }
    ...
    // Format all options for display (optional)
    $allOptions = array_map(function($item) use ($rate, $gain) {
        return [
            'pool' => $item['pool'],
            'price_usd' => (float) $item['price'],
            'price_naira' => round(((float) $item['price'] * $rate) + $gain, 2),
        ];
    }, $json);
    ...
    return response()->json([
        ...
        'rate'        => $rate,
        'gain'        => $gain,
        'all_options' => $allOptions, // Include all pricing options if needed
    ]);
}
