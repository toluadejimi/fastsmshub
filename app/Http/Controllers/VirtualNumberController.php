...
class VirtualNumberController extends Controller
{
    ...
    {
        $candidates = [
            $data['data']['tiers']              ?? null,  // NEW: check data.tiers first (VirtualApiController format)
            $data['tiers']                      ?? null,
...
{
    // Provider returns "price" (required for purchase). Some older formats may return api_price/amount/cost.
    $providerPrice = null;
    ...
    // This is ONLY for displaying / wallet check (your internal)
    $ngnPrice = round(($providerPrice * $this->usdToNgnRate()) + $gain, 2);

    return [
        'pool'      => $pool,
        'price'     => $providerPrice, // ✅ this is what you must send to /purchase EXACTLY
        'ngn_price' => $ngnPrice,
    ...
    public function purchase(Request $request)
    {
        ...
        try {
            ...
            // 4. Call provider API — send api_price (raw provider price, not display_price)
   $apiResponse = $this->api()->post("{$this->base}/purchase", [
    ...
    'price'        => $matchedPool['price'],
            ...
            Verification::create([
               ...
               'api_cost'      => $matchedPool['price'],   // was $matchedPool['api_price']
'price'         => $matchedPool['price'],   // was $matchedPool['api_price']
    ...
    // GET /virtual/check/{activationId}/poll
   public function poll(string $activationId)
{
    ...
    $existingCode = $sanitize($verification->code);

    // ── If we already have a real code in DB, NEVER hit the API again ──────
    // Just return it immediately regardless of status field
    if ($existingCode !== null) {
        // Make sure status is Completed in case it wasn't saved correctly
    ...
    try {
        ...
        $incomingCode   = $sanitize($apiResponse['data']['code'] ?? $apiResponse['code'] ?? null);

        // Only save if incoming is a real code
    ...
    public function cancel(string $activationId)
    {
        ...
        try {
            return DB::transaction(function () use ($activationId, $user) {
                ...
                // Refund amount — prefer naira_amount, fall back to price
