...
const _cancelTimers    = {};
const _cancelDeadlines = {}; // { [id]: epoch ms when lock expires }
...
function startCancelCountdown(id, remainingSeconds = 180) {
    // Store absolute deadline — immune to browser tab throttling
    ...
    _renderCountdown(id); // paint immediately
    _cancelTimers[id] = setInterval(() => _renderCountdown(id), 500);
    // 500ms tick — snappy re-sync after tab restore without extra cost
}

// Re-sync all active countdowns the moment the tab becomes visible again
...
function startPolling(id, remaining = 180) {
    ...
    // Start cancel countdown with correct remaining time
    startCancelCountdown(id, remaining);

    const interval = setInterval(async () => {
        try {
            const r = await fetch(`/virtual/check/${id}/poll`, { cache: 'no-store' });
            const j = await r.json();

            if (j.status === 'done' && j.code) {
                clearInterval(interval);

                // Stop cancel countdown
                if (_cancelTimers[id]) {
                    clearInterval(_cancelTimers[id]);
                    delete _cancelTimers[id];
                }
...
async function cancelOrder(id) {
    // Block cancel if code already received
...
// ── Auto-start polling + countdown for active orders on page load ──────────
...
(function() {
    ...
    let remaining = LOCK - age;

    if (remaining > 0) {
        startPolling(id, remaining);
    } else {
        // Lock expired — enable cancel immediately
        document.querySelectorAll(`[data-activation="${id}"] .cancel-btn`).forEach(btn => {
            btn.textContent = 'Cancel & Refund';
            btn.disabled = false;
            btn.classList.remove('opacity-50', 'cursor-not-allowed');
